 ...
 void print_mem_state(void)
{
    //For standard pool
    for (int i= 3; i<4; i++){
        mem_pool_t p = mem_pools[i];
        printf("\n Pool 4:\n");
        mem_std_free_block_t *k = p.start;
        mem_std_allocated_block_t *tmp;

        printf("[");
        while((void*)k != p.end)
        {
            if(is_block_free(&k->header))
            {
                for (int j = 0; j < get_block_size(&k->header); j++)
                {
                    printf("\033[1;32m");
                    printf(".");
                    printf("\033[0m");
                }
                k = (void*)((size_t)k + get_block_size(&k->header) + 16);
            }
            else
            {
                tmp = (void *)k;
                for (int j = 0; j < get_block_size(&tmp->header); j++)
                {
                    printf("\033[1;31m");
                    printf("X");
                    printf("\033[0m");
                }
                k = (void *)((size_t)tmp + 16 + get_block_size(&tmp->header));
            }
        }
        printf("]\n");      
    }    
